...
const bot = new TelegramBot(process.env.PAYBOY_TEST_TOKEN, {
  polling: true,
});
...
// Define the MongoDB schema for storing items from Payboy
const PayBoySchema = new mongoose.Schema({
...
const PayBoy = mongoose.model("PayBoy", PayBoySchema);
...
async function onMessage(msg) {
  ...
  let requestBody = {
    chat_history: [],
    company_info: {
      company_desc: "PayBoy is a payroll HR software.",
      company_name: "PayBoy",
      product_list: [""],
  ...
  const prevConvoArr = await PayBoy.find({ userId: userId });
